...
class select extends db{
    ...
    public function selectJoinThree($columns, $table1, $table2, $table3, $where, $sorttype, $limit, $inner_column_append = NULL){
        // initialize param values
        $bind_param_where = $this->iniParamvalues($where);
        ...
        $clause_app = ($where == NULL) ? 'WHERE' : 'AND ';
        $inner_column_append = ($inner_column_append == NULL) ? NULL : "$clause_app $inner_column_append";
        $sql = "SELECT $columns FROM $table1 INNER JOIN $table2 INNER JOIN $table3 $bind_param_where $inner_column_append $sorttype $limit";
        $this->query = $this->conn->prepare($sql);
